...
namespace SkulyRepack
{
    public partial class Form13 : Form
    {
                ...
                if ((sqlcheck2.Length == 0))
                {
                    ...
                    if (isNumeric)
                    {
                        ...
                        if (b == DialogResult.Yes)
                        {
                            ...
                            mysqldump.StartInfo.Arguments = "-u root --password=root acore_world gameobject --result-file=" + @"updater\launcher\sql\NodeRespawn\backup\NodeDump.sql";
                        ...
                        var spawntimestring = "/C " + @"MySQL\bin\mysql -u root --password=root acore_world < " + @"updater\launcher\sql\NodeRespawn\execute.sql";
        ...
        private void button2_Click(object sender, EventArgs e)
        {
            ...
            {
                var spawntime = new Process();
                var spawntimestring = "/C " + @"MySQL\bin\mysql -u root --password=root acore_world < " + @"updater\launcher\sql\NodeRespawn\backup\NodeDump_Original.sql";
                spawntime.StartInfo.FileName = "cmd.exe";
                spawntime.StartInfo.Arguments = spawntimestring;
                spawntime.StartInfo.WindowStyle = ProcessWindowStyle.Hidden;
                spawntime.Start();
